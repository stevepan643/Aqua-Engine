...
public class CoolCube {
  ...
  public static void main(String[] args) {
    ...
    window = new Window(width, height, "Cool Cube");
    ...
    Uniform<Matrix4f> viewUniform = new Uniform<Matrix4f>("view", camera.getViewMatrix());

    Uniform<Vector3f> viewPosUniform = new Uniform<Vector3f>("viewPos", camera.getPosition());

    Uniform<Vector3f> lightPosition =
        new Uniform<Vector3f>("light.position", new Vector3f(1.0f, 1.0f, -0.5f));
    Uniform<Vector3f> lightAmbient =
        new Uniform<Vector3f>("light.ambient", new Vector3f(0.2f, 0.2f, 0.2f));
    Uniform<Vector3f> lightDiffuse =
        new Uniform<Vector3f>("light.diffuse", new Vector3f(0.5f, 0.5f, 0.5f));
    Uniform<Vector3f> lightSpecular =
        new Uniform<Vector3f>("light.specular", new Vector3f(1.0f, 1.0f, 1.0f));

    Uniform<Float> timeUniform = new Uniform<Float>("move_time", 1.0f);

    mesh1.getTransform().translate(1.0f, 1.0f, -0.5f);
    ...
    while (!window.isShouldClose()) {
      ...
      glfwSetWindowTitle(window.get(), "Cool Cube - FPS: " + String.format("%.2f", fps));
