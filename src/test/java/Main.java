...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    // Initialization shader(create, compile link etc).
    Shader vertex = new Shader("src/main/resources/shaders/v.vs", GL_VERTEX_SHADER);
    Shader vertex2 = new Shader("src/main/resources/shaders/v2.vs", GL_VERTEX_SHADER);
    Shader fragment = new Shader("src/main/resources/shaders/f.fs", GL_FRAGMENT_SHADER);
    Shader fragment2 = new Shader("src/main/resources/shaders/f2.fs", GL_FRAGMENT_SHADER);
    Shader fragment3 = new Shader("src/main/resources/shaders/f3.fs", GL_FRAGMENT_SHADER);
    ...
    GameObject cube =
        new GameObject(material, ObjUtil.loadModel("src/main/resources/model/Cube.obj"));
    GameObject david =
        new GameObject(material, ObjUtil.loadModel("src/main/resources/model/David head.obj"));
    GameObject lady =
        new GameObject(
            material, ObjUtil.loadModel("src/main/resources/model/CREEPY_LADY_PG_EX2.obj"));
    GameObject nile =
        new GameObject(material, ObjUtil.loadModel("src/main/resources/model/nile.obj"));
    ...
    Uniform<Matrix4f> viewUniform = new Uniform<>("view", camera.getViewMatrix());

    Uniform<Vector3f> viewPosUniform = new Uniform<>("viewPos", camera.getPosition());

    mesh1.getTransform().translate(-1.0f, 2.0f, 1.5f).scale(0.05f);

    Uniform<Vector3f> lightPosition =
        new Uniform<>("light.position", new Vector3f(-1.5f, 2.0f, 2f));
    Uniform<Vector3f> lightAmbient = new Uniform<>("light.ambient", new Vector3f(0.1f, 0.1f, 0.1f));
    Uniform<Vector3f> lightDiffuse =
        new Uniform<>("light.diffuse", new Vector3f(1.0f / 1.2f, 0.85f / 1.2f, 0.72f / 1.2f));
    Uniform<Vector3f> lightSpecular =
        new Uniform<>("light.specular", new Vector3f(0.2f, 0.2f, 0.2f));
