...
/**
 * The {@code Window} class is responsible for creating and managing a window using the GLFW
 * library. It initializes the GLFW library, sets the necessary window hints for creating an OpenGL
 * context, and provides a method to check if the operating system is macOS.
 *
 * @author Steve Pan
 * @since 1.0
 */
public class Window {
  ...
  /**
   * Initializes the GLFW library and sets the window hints for creating an OpenGL context. This
   * method sets the OpenGL context version to 3.3 and uses the core profile. If the application is
   * running on macOS, it also sets the forward compatibility flag.
   *
   * @since 1.0
   */
  private void init() {
  ...
  public void switchFullScreen() {
    if (isFullScreen) {
      glfwSetWindowPos(window, (monitorWidth - 800) / 2, (monitorHeight - 600) / 2);
  ...
  /**
   * Swaps the front and back buffers of the specified window. This method is typically used to
   * update the display with the rendered content. It should be called after rendering operations
   * are completed.
   *
   * @since 1.0
   */
