...
public class Texture {
  ...
  public Texture(@Nonnull String filepath) {
    ...
    switch (type) {
      case jpg:
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA, width[0], height[0], 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        break;
      case png:
        glTexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA, width[0], height[0], 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
        break;
      default:
        break;
    }
  ...
  private TextureType typeCheck(@Nonnull String filepath) {
  ...
  /**
   * Binds the texture for use in OpenGL rendering. This method activates the texture unit specified
   * by the texture ID and binds the texture to the GL_TEXTURE_2D target.
   *
   * @since 1.0
   */
  public void use() {
